...
```
```{r, eval = TRUE} 
condition_anova <- aov(vignette_response ~ condition, na.action=na.exclude, data=data)
summary(condition_anova)
TukeyHSD(condition_anova)
```
```{r, eval = TRUE} 
age_anova <- aov(vignette_response ~ age_bracket, na.action=na.exclude, data=data)
summary(age_anova)
TukeyHSD(age_anova)
```
